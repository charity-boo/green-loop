...
import { AdminActionLogDoc, UserDoc } from '@/types/firestore';
...
function StateDelta({ before, after }: { before: Record<string, unknown> | null | undefined, after: Record<string, unknown> | null | undefined }) {
    ...
    return (
        <div className="flex flex-col gap-1 font-mono text-xs">
            {before && (
                <div className="text-muted-foreground line-through truncate max-w-[200px]" title={JSON.stringify(before)}>
        ...
        <div className="space-y-6">
            <header>
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight text-foreground">Governance & System Anomalies</h1>
                        <p className="text-muted-foreground">Immutable record of all administrative actions and quantified instability vectors.</p>
                    </div>
                </div>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="p-5 bg-card border border-border rounded-xl flex items-center justify-between gap-4">
                    <div>
                        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">Overrides (7d)</p>
                        <p className="text-3xl font-bold text-foreground">{metrics.totalOverrides7d}</p>
                    </div>
                ...
                <div className="p-5 bg-card border border-border rounded-xl flex items-center justify-between gap-4">
                    <div>
                        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">Deactivations (7d)</p>
                        <p className="text-3xl font-bold text-foreground">{metrics.totalStatusToggles7d}</p>
                    </div>
                ...
                <div className="p-5 bg-card border border-border rounded-xl flex items-center justify-between gap-4">
                    <div>
                        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">Active Under Review</p>
                        ...
                        {metrics.overrideTrend7d.map((day, idx: number) => {
                            ...
                            return (
                                <div key={idx} className="flex-1 flex flex-col justify-end group relative items-center">
                                    ...
                                    />
                                    <span className="text-[8px] text-muted-foreground mt-1 opacity-50 group-hover:opacity-100">{idx === 6 ? 'Today' : ''}</span>
                                </div>
                            );
                        })}
                    </div>
            ...
            <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-sm border-collapse">
                        <thead>
                            <tr className="text-left border-b border-border font-medium text-muted-foreground bg-muted/50">
                            ...
                            {logs.map((log) => (
                                <tr key={log.id} className="hover:bg-muted/50 transition-colors">
                                    ...
                                    <td className="py-4 px-6">
                                        <div className="font-medium text-foreground">{log.admin?.name || 'Unknown Admin'}</div>
                                        <div className="text-xs text-muted-foreground">{log.admin?.email || log.adminId}</div>
                                    </td>
                                    <td className="py-4 px-6">
                                    ...
                                    <td className="py-4 px-6 text-slate-600">
                                        ...
                                        <br />
                                        <span className="text-xs font-mono bg-slate-100 px-1 py-0.5 rounded text-muted-foreground" title={log.targetId}>
                                            {log.targetId.slice(0, 8)}...
                                        </span>
                                    </td>
                                    <td className="py-4 px-6 text-slate-600">
                                        {log.reason ? (
                                            <div className="max-w-[200px] text-xs leading-relaxed italic text-muted-foreground" title={log.reason}>
                                                &quot;{log.reason}&quot;
                                            </div>
                                        ) : (
