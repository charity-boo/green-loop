...
type ActiveTab = 'issues' | 'ai-review' | 'messages';
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Review user-submitted reports, inquiries, and oversee AI waste classification.</p>
        </div>

        <div className="flex p-1.5 bg-slate-100 rounded-2xl border border-border shadow-inner overflow-x-auto no-scrollbar">
          <button 
            onClick={() => { setActiveTab('issues'); setStatusFilter('PENDING'); }}
            className={cn(
              "flex items-center gap-2.5 px-6 py-2 rounded-xl text-sm font-bold transition-all whitespace-nowrap",
              activeTab === 'issues' 
                ? "bg-card text-foreground shadow-lg ring-1 ring-slate-200" 
          ...
          <button 
            onClick={() => { setActiveTab('messages'); setStatusFilter('NEW'); }}
            className={cn(
              "flex items-center gap-2.5 px-6 py-2 rounded-xl text-sm font-bold transition-all whitespace-nowrap",
              activeTab === 'messages' 
                ? "bg-card text-foreground shadow-lg ring-1 ring-slate-200" 
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 border-b border-border pb-6">
        <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto no-scrollbar pb-2 md:pb-0">
          <div className="flex items-center gap-2 mr-4 bg-muted/50 px-3 py-1.5 rounded-xl border border-border text-slate-400 font-bold text-[10px] uppercase tracking-widest shrink-0">
          ...
          {(activeTab === 'issues' ? [
            ...
            { label: 'Replied', value: 'REPLIED', icon: ShieldCheck },
            { label: 'All', value: 'ALL', icon: Layers },
          ] : [
        ...
        <div className="relative w-full md:w-80">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-muted/50 border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all text-xs font-medium"
          />
        </div>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-32">
          ...
          <p className="mt-6 text-muted-foreground font-bold text-sm tracking-wide animate-pulse">Scanning Secure Repository...</p>
        </div>
      ) : (
          ...
          {activeTab === 'issues' ? (
            ...
            messages.length > 0 ? (
              messages.map(message => (
                <ContactMessageCard key={message.id} message={message} onUpdate={fetchData} />
              ))
            ) : (
              <EmptyState title="No messages found" desc="Your inbox is clear! Good job." />
            )
          ) : (
...
function EmptyState({ title, desc }: { title: string, desc: string }) {
  return (
    <div className="col-span-full py-24 flex flex-col items-center justify-center bg-muted/50 border-2 border-dashed border-border rounded-3xl">
      <div className="w-20 h-20 bg-card rounded-2xl flex items-center justify-center shadow-lg border border-border mb-6">
        <LayoutGrid className="w-10 h-10 text-slate-200" />
      </div>
      <h3 className="text-xl font-bold text-foreground mb-2">{title}</h3>
      <p className="text-muted-foreground text-sm max-w-xs text-center">{desc}</p>
    </div>
  );
}
