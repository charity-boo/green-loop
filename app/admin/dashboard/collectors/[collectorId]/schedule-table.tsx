...
export default function ScheduleTable({ data, meta, baseUrl = "", searchParams = {} }: Props) {
    ...
    const statusColors: Record<Status, string> = {
        PENDING: "bg-slate-100 text-slate-700 border-border",
        COMPLETED: "bg-emerald-100 text-emerald-800 border-emerald-200",
        MISSED: "bg-red-100 text-red-800 border-red-200"
    };

    return (
        <div className="bg-card rounded-xl shadow-sm space-y-4">
            <table className="w-full text-sm border-collapse">
                <thead>
                    <tr className="text-left border-b font-medium text-muted-foreground bg-muted/50 rounded-t-xl">
                ...
                <tbody>
                    {data.map((s) => (
                        <tr key={s.id} className="border-b hover:bg-muted/50 transition-colors">
                            ...
                            <td className="py-4 px-6 text-right font-medium text-foreground border-r border-border">
            ...
            {/* Pagination Controls */}
            <div className="flex justify-between items-center p-4 border-t bg-muted/50 rounded-b-xl">
                <Link
                    href={buildLink(Math.max(meta.currentPage - 1, 1))}
                    className={`px-4 py-2 text-sm font-medium rounded-md border bg-card text-slate-700 hover:bg-muted/50 transition-colors ${meta.currentPage <= 1 ? "opacity-50 pointer-events-none cursor-not-allowed" : ""
                        }`}
                ...
                <Link
                    ...
                    className={`px-4 py-2 text-sm font-medium rounded-md border bg-card text-slate-700 hover:bg-muted/50 transition-colors ${meta.currentPage >= meta.totalPages || meta.totalPages === 0
