...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (userData.region) {
      const eligibleSchedules = await adminDb
      ...
      for (const scheduleDoc of eligibleSchedules.docs) {
        // Trigger the engine for each eligible schedule
        const result = await assignScheduleAutomatically(scheduleDoc.id);
