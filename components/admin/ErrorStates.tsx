...
export function ChartErrorState({ title, onRetry }: ChartErrorStateProps) {
    return (
        <div className="bg-white rounded-xl shadow-sm p-6 w-full h-[400px] border border-red-100 flex items-center justify-center">
...
export function KPIErrorState() {
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(3)].map((_, i) => (
                <div key={i} className="p-6 bg-white rounded-2xl border border-red-50 flex items-center gap-4">
                    <div className="p-3 bg-red-50 rounded-xl text-red-500">
                        <AlertCircle className="w-6 h-6" />
                    </div>
                    <div>
                        <p className="text-sm font-medium text-slate-500 uppercase tracking-wider">Metric Cluster {i + 1}</p>
